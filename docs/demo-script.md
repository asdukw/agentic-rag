...
展示最终答案下方的 citation、chunk 原文、实体/关系命中和 NetworkX 路径。强调回答模型
只能接收已经选中的证据，citation 必须是允许 chunk ID 的子集；点击 trace/replay 可查看
本次的路由分数、路径和最终上下文，无需重新调用 embedding 或模型。
